...
export default function BidForm(props) {
    ...
    const handleFileUpload = ( event ) => {
        ...
        data.append('description', event.target.files[0].type + ';' + event.target.files[0].size)
    ...
    const sendFileToServer = () => {
        ...
        axios.post('http://localhost:3003/upload', values.fileData, {
            headers: {
        ...
        .then(response => {
            ...
            setValues({...values, linkToFile: `http://localhost:3003/pdf/${response.data.file.filename}` })
    ...
    const handleSubmit = () => {
        ...
        axios.post('http://localhost:3003/api/bids', {
            name: values.name,
            email: values.email,
            fileInfo: values.linkToFile,
            country: props.countryName
