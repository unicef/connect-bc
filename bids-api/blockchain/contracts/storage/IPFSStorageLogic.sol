...
contract IPFSStorageLogic is Storage {
    ...
    /// @param _key The key for the record
    function getIPFSHash(address _factProvider, bytes32 _key) external view returns (bool success, string memory value) {
        return _getIPFSHash(_factProvider, _key);
    }

    function _setIPFSHash(bytes32 _key, string memory _value) allowedFactProvider internal {
    ...
    function _getIPFSHash(address _factProvider, bytes32 _key) internal view returns (bool success, string memory value) {
